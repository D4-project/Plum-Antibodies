...
Plum Island applies computed tags to scan documents through the rules maintained
in this repository.
...
Tag rules are stored under `tags/` as YAML files.
...
`Plum-Island/tools/tag_mgmt.py import` imports YAML rules into the Plum Island
SQLite database.
...
The commands below are run from the root of a checked-out Plum-Island
repository. The YAML files are available there through the submodule at
`webapp/tags/tags/`.

Import every YAML rule:
...
After rules change, tags can be recomputed with `Plum-Island/tools/tag_mgmt.py reindex`.
